...
{{--                                <h3 class="card-title"><span style="padding-right: 10px;margin-left: 10px;"><i class="fas fa-book" style="border-radius: 50%; padding: 15px; background: #3d807a; color: #ffffff"></i></span>Total Found : {{ $chartOfAccounts->count() }}</h3>--}}
                            ...
                            <div class="row">
                                <div>
                                    <a href="{{ route('coa.create') }}" class="btn btn-info btn-sm" style="margin-top: 10px; margin-left: 10px;"> <i class="fas fa-plus-circle"></i>
                                        {{ __('New')}}</a>
                                </div>
                            </div>
                        </div>
...
{{--                                        {{ $coa->is_enabled == 1 ? 'Active' : 'Inactive' }} <br>--}}
                                    ...
                                    <td>
                                        @if($coa->is_delete == 0)
                                        {{ Form::open(['route'=>['coa.destroy',$coa->id],'method'=>'delete','onsubmit'=>'return confirmDelete()']) }}
    ...
    <script>
        function statusChange(id){
            ...
            $.ajax({
                ...
                location.replace("{{ route('coa.name') }}");
            })
        }
    </script>
    <script>
