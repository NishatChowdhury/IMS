                            ...
                            <table id="example2" class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    @foreach ($syllabuses as $syllabus)
                                        <tr>
                                            <td>{{ $i++ }}</td>
                                            <td>{{ $syllabus->academicClass->academicClasses->name ?? '' }}&nbsp;{{ $syllabus->academicClass->section->name ?? '' }}{{ $syllabus->academicClass->group->name ?? '' }}
                                            </td>
                                            <td>{{ $syllabus->academicClass->sessions->year ?? '' }}</td>
                                            <td>{{ $syllabus->title }}</td>
                                            <td>
                                                <a href="{{ asset('assets/syllabus') }}/{{ $syllabus->file }}"
                                                    class="btn btn-success btn-sm"
                                                    target="_blank">{{ __('View Syllabus') }} <i
                                                        class="fas fa-eye"></i></a>
                                            </td>
                                            <td>
                                                {{ Form::open(['route' => ['syllabus.delete', $syllabus->id], 'method' => 'delete', 'onsubmit' => 'return confirmDelete()']) }}
    ...
    <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                {{ Form::open(['route' => 'syllabus.store', 'method' => 'post', 'files' => true]) }}
                ...
                <div class="modal-body">
                    <div class="form-group row">
                        <label for="" class="col-sm-2 col-form-label"
                            style="font-weight: 500; text-align: right">Academic Class</label>
                        <div class="col-sm-10">
                            <div class="input-group">
                                <select name="academic_class_id" id="inputState" class="form-control">
                                    <option value=""> -- Select Academic Class -- </option>
                                    @foreach ($academic_class as $value)
                                        <option value="{{ $value->id }}"> {{ $value->academicClasses->name ?? '' }}
                                            {{ $value->section->name ?? '' }}{{ $value->group->name ?? '' }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="form-group row">
                        <label for="" class="col-sm-2 col-form-label"
                            style="font-weight: 500; text-align: right">Syllabus (pdf)*</label>
                        <div class="col-sm-10">
