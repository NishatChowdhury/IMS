...
<div class="wrapper">
    ...
    <!-- Navbar -->
    <nav class="main-header navbar navbar-expand bg-white navbar-light border-bottom" style="margin-left: 0px">
        <a href="{{ route('teacher.dashboard') }}" class="btn btn-dark btn-sm">Dashboard</a>
        <!-- Left navbar links -->
                ...
                <ul class="dropdown-menu dropdown-menu-lg dropdown-menu-right" style="background-color: #ffffff; padding: 5px; border-radius: 10px">
                    <li class="user-header">
                        ...
                        <p class="user-hd-text">
                            {{ strtoupper(Auth::guard('teacher')->user()->name) }} <br>
                            {{--                    {{ Auth::user()->role_id }} <br>--}}
                            <small>Member since {{ Auth::guard('teacher')->user()->created_at->format('M. Y') }}</small>
                        </p>
                    </li>
                    <!-- Menu Body -->
